...
Automated job digest for fresher-friendly IT roles in Ho Chi Minh City. The bot fetches matching JD listings from ITviec, filters out senior or unrelated roles, sends a clean Telegram digest every weekday morning, and remembers recently sent jobs to avoid duplicates.

## Demo

![Telegram digest preview](docs/demo-placeholder.svg)

> Placeholder preview. Replace this image with a real Telegram screenshot when the first polished digest is ready.
...
- HCM-focused filtering with senior and irrelevant-role exclusion.
...
flowchart LR
    ...
    C --> D[Parse and Filter Jobs]
    D --> E[Deduplicate Against<br/>data/seen_jobs.json]
    E --> F[Format Telegram Digest]
    F --> G[Telegram Bot API]
    G --> H[Telegram Chat]
    E --> I[Update seen_jobs.json]
    I --> J[Commit State Back<br/>to GitHub]
```
...
- Add a real Telegram screenshot after the first production digest.
